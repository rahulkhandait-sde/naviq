function filterGraphData(jsonData, shortestPathNodeIds = []) {
  const pathNodeIds = new Set()

  if (shortestPathNodeIds && shortestPathNodeIds.length > 0) {
    shortestPathNodeIds.forEach((pathItem) => {
      if (typeof pathItem === "string") {
        // Extract node IDs from format like "nodeId1 → nodeId2 (distance)"
        const matches = pathItem.match(/([a-f0-9]+)/g)
        if (matches) {
          matches.forEach((id) => pathNodeIds.add(id))
        }
      }
    })
  }
  ...
  function addNode(node) {
    if (pathNodeIds.size === 0 || pathNodeIds.has(node._id)) {
      nodes[node._id] = node._id
    }
  }

  function addConnection(conn) {
    const fromInPath = pathNodeIds.size === 0 || pathNodeIds.has(conn.from._id)
    const toInPath = pathNodeIds.size === 0 || pathNodeIds.has(conn.to._id)

    if (fromInPath && toInPath) {
      if (!edges[conn.from._id]) edges[conn.from._id] = []
      if (!edges[conn.to._id]) edges[conn.to._id] = []
      edges[conn.from._id].push({ node: conn.to._id, distance: conn.distance })
      edges[conn.to._id].push({ node: conn.from._id, distance: conn.distance })
    }
  }
  ...
  if (jsonData.buildings) {
    jsonData.buildings.forEach((building) => {
      ...
      // Process floors within buildings
      if (building.floors) {
  ...
  console.log("Total nodes after filtering:", Object.keys(nodes).length)
  console.log("Total edges after filtering:", Object.keys(edges).length)

  return { nodes, edges }
}

export function dijkstraWithSteps(jsonData, startId, endId) {
  const { nodes, edges } = filterGraphData(jsonData)
  ...
  while (unvisited.size > 0) {
    ...
    unvisited.delete(u)
    visited.add(u)
    ;(edges[u] || []).forEach(({ node: v, distance }) => {
      if (visited.has(v)) return

      const alt = distances[u] + distance
      if (alt < distances[v]) {
        distances[v] = alt
        prev[v] = { id: u, distance }
      }
    })
  }
  ...
  let u = endId
  while (prev[u]) {
    const from = prev[u].id
    const distance = prev[u].distance
    steps.unshift(`${nodes[from]} → ${nodes[u]} (${distance.toFixed(2)} m)`)
    u = from
  }
...
// Test the function
// const path = dijkstraWithSteps(jsonData, "68a61f20e5f5a762fbaf842d", "68a60b83e5f5a762fbaf832b")
// console.log("Path:", path)
// console.log("If no path found, check if these nodes are connected in the graph")
